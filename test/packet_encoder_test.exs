defmodule PacketEncoderTest do
  ...
  describe "PacketEncoder" do
    ...
    test "size flag" do
      ...
      assert(buffer == <<224, 1, 255, fill_binary(1, 255)::binary>>)
      assert Queue.empty?(encoder.packets)
      assert Queue.empty?(encoder.buffers)
      ...
      assert(buffer == <<192, 1, 0, 1, 0, fill_binary(1, 256)::binary>>)
      assert Queue.empty?(encoder.packets)
      assert Queue.empty?(encoder.buffers)
    end
    ...
    test "multiple packets" do
      ...
      packet_1 = <<224, 1, 25, fill_binary(1, 25)::binary>>
      packet_2 = <<224, 1, 30, fill_binary(2, 30)::binary>>
      packet_3 = <<192, 1, 0, 1, 188, fill_binary(3, 444)::binary>>
