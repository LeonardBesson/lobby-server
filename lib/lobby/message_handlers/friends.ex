defmodule Lobby.MessageHandlers.Friends do
  ...
  def handle(:add_friend_request, msg, %State{conn: conn, user: user} = state) do
    response =
  ...
  def handle(:fetch_pending_friend_requests, msg, %State{conn: conn, user: user} = state) do
    update_friend_requests(user.id)
    state
  end

  def handle(:friend_request_action, msg, %State{conn: conn, user: user} = state) do
    response =
  ...
  def handle(:fetch_friend_list, msg, %State{conn: conn, user: user} = state) do
    update_friend_list(user.id)
    state
  end

  def handle(:remove_friend, msg, %State{conn: conn, user: user} = state) do
